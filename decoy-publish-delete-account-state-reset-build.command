...
echo "Publishing Decoy Wallet delete-account state reset fix..."
echo "Repository: $REPO"
echo "Current branch: $(git branch --show-current)"
echo "Starting commit: $(git rev-parse HEAD)"
echo "This app update resets local access display state after account deletion."
echo "It does not touch emergency alert infrastructure, payment backends, signing, or entitlements."
...
git add \
  ...
  decoy-publish-delete-account-state-reset-build.command

if git diff --cached --quiet; then
  echo "No staged delete-account state reset changes found; using existing HEAD."
else
  git commit -m "fix: reset access state after account deletion"
fi
...
echo "Done. Delete-account state reset TestFlight build started."
echo "Next: install TestFlight, delete/recreate same-email disposable account, and confirm the Unlock overlay appears before payment."
