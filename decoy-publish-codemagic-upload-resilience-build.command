...
echo "Publishing CodeMagic App Store upload resilience fix..."
echo "Repository: $REPO"
echo "Current branch: $(git branch --show-current)"
echo "Starting commit: $(git rev-parse HEAD)"
echo "This changes only codemagic.yaml upload handling."
echo "It does not touch app behavior, payments, alerts, backend code, signing files, or entitlements."

git add \
  codemagic.yaml \
  decoy-publish-codemagic-upload-resilience-build.command

if git diff --cached --quiet; then
  echo "No staged CodeMagic upload resilience changes found; using existing HEAD."
else
  git commit -m "fix: tolerate App Store upload success after Apple 500"
fi
...
echo "Done. CodeMagic upload resilience TestFlight build started."
