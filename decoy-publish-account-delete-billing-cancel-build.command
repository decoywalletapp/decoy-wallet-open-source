...
echo "Publishing Decoy Wallet account-delete billing cancellation app update..."
echo "Repository: $REPO"
echo "Current branch: $(git branch --show-current)"
echo "Starting commit: $(git rev-parse HEAD)"
echo "Last good tested mobile commit before this fix: 0ac966d7dceb47f98d435c0fb994cb59512d9604"
echo "This app update changes only account deletion routing/text."
echo "It does not touch emergency alert infrastructure."

git add \
  codex_backend_work/decoy-stripe-webhook-live/Dockerfile \
  ...
  decoy-deploy-account-delete-billing-cancel-backend.command \
  decoy-publish-account-delete-billing-cancel-build.command \
  lib/custom_code/actions/delete_current_user_account.dart \
  lib/settings_pages/delete_user_account/delete_user_account_widget.dart

if git diff --cached --quiet; then
  echo "No staged account-delete billing changes found; using existing HEAD."
else
  git commit -m "fix: cancel stripe billing before account deletion"
fi
...
echo "Done. Account-delete billing cancellation TestFlight build started."
echo "Next: wait for TestFlight, then test account deletion with a disposable user."
