...
echo "Publishing Decoy Wallet renewal push tap navigation update..."
echo "Repository: $REPO"
echo "Current branch: $(git branch --show-current)"
echo "Starting commit: $(git rev-parse HEAD)"
echo "This is app-only push tap routing polish for subscription renewal notifications."
...
git diff --check -- \
  ...
  lib/custom_code/actions/init_push_tap_listener.dart \
  decoy-publish-renewal-push-tap-build.command
...
if git diff --cached --quiet; then
  echo "No staged renewal push tap changes found; using existing HEAD."
else
  git commit -m "fix: open renewal push taps from unlocked app"
fi
...
echo "Done. Renewal push tap TestFlight build started."
echo "Next: install TestFlight, send/tap one subscription renewal push, and confirm Manage Subscription opens."
