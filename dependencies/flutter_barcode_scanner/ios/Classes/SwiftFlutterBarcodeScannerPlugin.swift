...
public class SwiftFlutterBarcodeScannerPlugin: NSObject, FlutterPlugin, ScanBarcodeDelegate,FlutterStreamHandler {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        if let rootViewController = activeRootViewController() {
        ...
        // Decoy only uses one-shot scans. Registering the continuous scan event
        // stream during iOS implicit engine startup can crash before Dart runs.
    }
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        if let rootViewController = SwiftFlutterBarcodeScannerPlugin.activeRootViewController() {
            SwiftFlutterBarcodeScannerPlugin.viewController = rootViewController
        }
