...
DART_BIN="/Users/mitchellwleblanc/dev/flutter/bin/cache/dart-sdk/bin/dart"

cd "$REPO"

echo "Publishing Decoy Wallet release hygiene update..."
echo "Repository: $REPO"
echo "Current branch: $(git branch --show-current)"
echo "Starting commit: $(git rev-parse HEAD)"
echo "This is app-only release cleanup."
echo "It removes exposed test routes, disables router diagnostics, and removes an unused debug signup action."
echo "It does not touch emergency alert infrastructure, payment backends, Supabase functions, signing, or trigger gates."
...
git diff --check -- \
  ...
  lib/flutter_flow/nav/nav.dart \
  lib/custom_code/actions/index.dart \
  lib/custom_code/actions/debug_sign_up.dart \
  decoy-publish-release-hygiene-build.command
...
if git diff --cached --quiet; then
  echo "No staged release hygiene changes found; using existing HEAD."
else
  git commit -m "chore: tighten release hygiene"
fi
...
echo "Done. Release hygiene TestFlight build started."
echo "Next: install TestFlight and do a short smoke check: open app, unlock with PIN, open Settings, open Manage Access, and confirm no obvious routing issue."
