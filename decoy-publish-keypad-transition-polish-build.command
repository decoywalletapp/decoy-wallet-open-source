...
echo "Publishing Decoy Wallet keypad and transition polish update..."
echo "Repository: $REPO"
echo "Current branch: $(git branch --show-current)"
echo "Starting commit: $(git rev-parse HEAD)"
echo "This is app-only UI polish for password reset keyboard focus and Auth Router to PIN transition timing."
...
git diff --check -- \
  ...
  decoy-publish-keypad-transition-polish-build.command
...
if git diff --cached --quiet; then
  echo "No staged keypad/transition polish changes found; using existing HEAD."
else
  git commit -m "fix: polish password reset focus and auth transition"
fi
...
echo "Done. Keypad/transition polish TestFlight build started."
echo "Next: install TestFlight, then test password reset keyboard Next and app-open Auth Router to PIN transition feel."
