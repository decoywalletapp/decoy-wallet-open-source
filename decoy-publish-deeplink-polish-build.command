...
echo "Publishing Decoy Wallet deep-link polish update..."
echo "Repository: $REPO"
echo "Current branch: $(git branch --show-current)"
echo "Starting commit: $(git rev-parse HEAD)"
echo "This is app-only polish for payment return and password reset/create copy."
...
git diff --check -- \
  ...
  lib/custom_code/actions/supa_recovery_update_password.dart \
  lib/custom_code/actions/supa_update_password.dart \
  ...
  decoy-publish-deeplink-polish-build.command
...
if git diff --cached --quiet; then
  echo "No staged deep-link polish changes found; using existing HEAD."
else
  git commit -m "fix: polish deep link return flows"
fi
...
echo "Done. Deep-link polish TestFlight build started."
echo "Next: install TestFlight, then check payment return logo/Refresh timing and password reset with an 11-character password."
